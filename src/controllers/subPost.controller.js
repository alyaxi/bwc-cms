...
const getAllSubPosts = async (req, res) => {
	try {
		...
		const subPosts = await subPostService.getAllSubPosts(id);
...
const getSubPostById = async (req, res) => {
	try {
		...
		if (!subPost)
			return res.status(404).json({ error: 'Sub-post not found' });
...
const updateSubPost = async (req, res) => {
	try {
		const updatedSubPost = await subPostService.updateSubPost(
			req.params.id,
			req.body
		);
